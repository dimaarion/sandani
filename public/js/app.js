...
var App = /*#__PURE__*/function () {
  function App() {
    ...
    _defineProperty(this, "namesLength", 0);
    _defineProperty(this, "telLength", 0);
    _defineProperty(this, "textLengtn", 0);
  }
  _createClass(App, [{
    key: "menuActive",
    ...
    key: "noinvalid",
    value: function noinvalid(name, tel) {
    ...
    key: "keyupPress",
    value: function keyupPress(name, tel) {
      var _this2 = this;
      $("#" + name).keyup(function (e) {
        _this2.namesLength = e.target.value.length;
        _this2.invalid(name, e.target.value.length);
      });
      $("#" + tel).keyup(function (e) {
        _this2.telLength = e.target.value.length;
        _this2.invalid(tel, e.target.value.length);
      });
    }
  }, {
    key: "isMessage",
    ...
    key: "mainMessage",
    value: function mainMessage(name, tel, submit) {
      var _this3 = this;
      this.keyupPress(name, tel);
      $("#" + submit).click(function () {
        var message = false;
        if (_this3.isMessage()) {
          axios__WEBPACK_IMPORTED_MODULE_2___default().post('/application', {
            name: $("#" + name).val(),
            tel: $("#" + tel).val()
        ...
        _this3.noinvalid(name, tel);
      });
    }
  }, {
    key: "reviewsAdd",
    value: function reviewsAdd(name, text, submit) {
      var _this4 = this;
      this.keyupPress(name, text);
      $('#' + submit).click(function () {
        if (_this4.isMessage()) {
          axios__WEBPACK_IMPORTED_MODULE_2___default().post('/otzivi', {
            name: $('#' + name).val(),
            text: $('#' + text).val()
          }).then(function (response) {
    ...
    value: function map() {
      ...
      function init() {
        myMap = new ymaps.Map("map", {
          center: [46.829457, 29.483857],
          // Координаты центра карты
          zoom: 17 // Маштаб карты
        });

        myMap2 = new ymaps.Map("map2", {
          center: [46.851210, 29.466364],
          // Координаты центра карты
          zoom: 17 // Маштаб карты
        });

        myMap.controls.add(new ymaps.control.ZoomControl() // Добавление элемента управления картой
        );

        myMap2.controls.add(new ymaps.control.ZoomControl() // Добавление элемента управления картой
        );

        var myPlacemark = new ymaps.Placemark([46.829457, 29.483857], {
        ...
        var myPlacemark2 = new ymaps.Placemark([46.851210, 29.466364], {
          // Координаты метки объекта
    ...
    value: function display() {
      ...
      this.slider();
      if (document.baseURI.split("/").filter(function (f) {
        return f === 'admin';
      }).length === 0) {
        this.map();
      }
